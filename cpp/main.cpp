...
// FILES TO READ AND WRITE
    std::ifstream in("examples/poly2d-cont000");
    std::ofstream out("examples/out_poly2d-cont000");
    std::ofstream out_intersection("examples/poly_intersection");
...
//SPOILING ANGELS OF POLYGON
    const double buffer_distance = 1.75;
...
#if 0
// FINDING RESULT DIFFERENCE
    boost::geometry::clear(in_polygon);
    std::ranges::for_each(in_contour, [&in_polygon](Point& p) { boost::geometry::append(in_polygon.outer(), p);});
    if(!boost::geometry::is_valid(in_polygon, message)){
        std::cout << "input polygon is NOT valid " << message << std::endl;
        std::cout << "trying to correct..." << std::endl;
    ...
    std::vector<boost::geometry::model::polygon<Point, false, true, std::vector>> dif_v;
    boost::geometry::sym_difference(in_polygon, out_polygon, dif_v);
    auto [area, perimeter] = write_intersection(dif_v, out_intersection);
    std::cout << "area : " << area << std::endl;
    std::cout << "perimeter : " << perimeter << std::endl;
    std::cout << "ratio : " << area / perimeter << std::endl;
#endif
}
